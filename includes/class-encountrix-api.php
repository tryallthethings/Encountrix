...
class EncountrixApi {
	...
	private array $expansions = array(
		11 => array(
			'name'          => 'Midnight',
			'category_name' => 'Midnight Raid',
		),
		10 => array(
			'name'          => 'The War Within',
			'category_name' => 'The War Within Raid',
		),
		9  => array(
			'name'          => 'Dragonflight',
			'category_name' => 'Dragonflight Raid',
		),
		8  => array(
			'name'          => 'Shadowlands',
			'category_name' => 'Shadowlands Raid',
		),
		7  => array(
			'name'          => 'Battle for Azeroth',
			'category_name' => 'Battle for Azeroth Raid',
		),
	);

	/** @var array<string, array{oauth: string, api: string}> */
	private array $regional_endpoints = array(
		'us' => array(
	...
	private array $achievement_category_ids = array(
		'Midnight'               => 15566,
	...
	private array $boss_name_overrides = array(
	 ...
	 */
	private array $tier_journal_map = array(
		'tier-mn-1' => array( 'Voidspire', 'Dreamrift', 'March on Quel\'Danas' ),
	);
	...
	private function check_rate_limit(): bool {
		...
		$rate_data['requests'] = array_filter(
			$rate_data['requests'],
			static function ( int $timestamp ) use ( $current_time ): bool {
	...
	public function fetch_raid_data(
		...
		try {
			...
			if ( ! in_array( $difficulty, array( 'normal', 'heroic', 'mythic' ), true ) ) {
			...
			if ( $response_code !== 200 ) {
				...
				return new WP_Error(
					'api_http_error',
					sprintf( __( 'Raider.io API error (HTTP %1$d): %2$s', 'encountrix' ), $response_code, $error_message )
				);
			}
	...
	private function parse_api_error( string $body, int $code ): string {
		...
		return match ( true ) {
			...
			default => sprintf(
				__( 'API Error (HTTP %1$d): %2$s', 'encountrix' ),
				$code,
	...
	public function get_raid_icon_id( string $raid_slug, int $expansion_id ): int|false {
		...
		$q = new WP_Query(
			array(
				...
				'meta_query'     => array(
					array(
						'key'   => '_encountrix_icon',
						'value' => 'raid',
					),
					array(
						'key'   => '_encountrix_raid',
						'value' => $raid_slug,
					),
				),
				'fields'         => 'ids',
