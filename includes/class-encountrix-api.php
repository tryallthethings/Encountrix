...
class EncountrixAPI {
	...
	private int $icon_retry_interval = 1800;
		...
		try {
			if (function_exists('encountrix_should_use_cron') && encountrix_should_use_cron() && !defined('DOING_CRON')) {
				$request_key = md5(serialize([
					'raid' => $raid, 'difficulty' => $difficulty, 'region' => $region,
					'realm' => $realm, 'guilds' => $guilds, 'limit' => $limit, 'page' => $page
				]));
			...
			$params = [
				'access_key' => $api_key, 'raid' => $raid, 'difficulty' => $difficulty,
				'region' => $region, 'limit' => $limit, 'page' => $page,
			];
			if (!empty($realm)) { $params['realm'] = $realm; }
			if (!empty($guilds)) { $params['guilds'] = $guilds; }
			...
			if (!empty($data['raidRankings']) && !empty($guilds) && $region !== 'world') {
				...
				if ($cached_world_rank !== false) {
					...
					$world_params = [
						'access_key' => $api_key, 'raid' => $raid, 'difficulty' => $difficulty,
						'region' => 'world', 'guilds' => $guilds, 'limit' => 1, 'page' => 0
					];
		...
		try {
			...
			foreach ($data['achievements'] as $ach) {
				if (isset($ach['name']) && str_starts_with($ach['name'], 'Mythic:')) {
					...
					$all_mythic_achievements[] = ['id' => $ach['id'], 'boss_name' => $boss_name];
			...
			if (!is_wp_error($static_data) && isset($static_data['raids'])) {
				foreach ($static_data['raids'] as $raid) {
					if ($raid['slug'] === $raid_slug) { $raid_info = $raid; break; }
				}
			}
	...
	public function get_boss_icon_id(string $boss_slug, string $raid_slug, ?string $boss_name = null, ?int $expansion_id = null): int|false {
		...
		if (get_transient($lock_key) !== false) {
			...
			for ($i = 0; $i < 20; $i++) {
				...
				if ($cached_id !== false && $cached_id > 0) { return (int) $cached_id; }
			}
			return false;
		}
		...
		$cached_id = get_transient($cache_key);
		if ($cached_id !== false) {
			if ($cached_id === 0) { return false; }
			if (get_post($cached_id)) { return (int) $cached_id; }
		...
		$attachment_id = $wpdb->get_var($wpdb->prepare(
			"SELECT p.ID FROM {$wpdb->posts} p
			 ...
			 AND pm1.meta_key = '_encountrix_boss' AND pm1.meta_value = %s
			 AND pm2.meta_key = '_encountrix_raid' AND pm2.meta_value = %s
			 LIMIT 1",
			$boss_slug, $raid_slug
		));
		...
		$attachment_id = $wpdb->get_var($wpdb->prepare(
			"SELECT ID FROM {$wpdb->posts}
			 WHERE post_type = 'attachment' AND post_status = 'inherit' AND post_name = %s LIMIT 1",
			$expected_filename
		));
	...
	public function get_all_boss_icon_ids(string $raid_slug): array {
		...
		$results = $wpdb->get_results($wpdb->prepare(
			"SELECT pm_boss.meta_value AS boss_slug, pm_boss.post_id
			 ...
			 INNER JOIN {$wpdb->postmeta} pm_raid ON pm_boss.post_id = pm_raid.post_id
			 INNER JOIN {$wpdb->posts} p ON p.ID = pm_boss.post_id
		...
		$map = [];
		foreach ($results as $row) { $map[$row->boss_slug] = (int) $row->post_id; }
		return $map;
	}
	...
	public function import_single_boss_icon(string $boss_slug, string $boss_name, string $raid_slug, int $expansion_id): int|bool {
		try {
			...
			if (!$this->wait_for_rate_limit(10)) { return false; }

			$response = wp_remote_get($media_url, [
				'timeout' => $this->http_timeout,
				'headers' => array_merge($this->base_headers(), ['Authorization' => 'Bearer ' . $token]),
			]);
			...
			if (isset($data['assets'])) {
				foreach ($data['assets'] as $asset) {
					if ($asset['key'] === 'icon') { $icon_url = $asset['value']; break; }
				}
			}
	...
	public function delete_all_icons(): int {
		...
		$attachment_ids = $wpdb->get_col(
			"SELECT DISTINCT post_id FROM {$wpdb->postmeta}
			 WHERE meta_key IN ('_encountrix_icon','_encountrix_raid','_encountrix_boss','_encountrix_expansion')"
		);
		...
		$this->delete_transients_by_prefix([
			'_transient_encountrix_icon_', '_transient_timeout_encountrix_icon_',
			'_transient_encountrix_blizzard_', '_transient_timeout_encountrix_blizzard_',
			'_transient_encountrix_request_lock_', '_transient_timeout_encountrix_request_lock_',
		]);
	...
	private function parse_api_error(string $body, int $code): string {
		...
		$error_detail = '';
		if (isset($data['error'])) {
			$error_detail = is_array($data['error']) ? ($data['error']['message'] ?? json_encode($data['error'])) : $data['error'];
		} elseif (isset($data['message'])) {
			$error_detail = $data['message'];
		}

		return match (true) {
			$code === 401 => sprintf(__('API authentication failed. Please check your API key. Details: %s', 'encountrix'), $error_detail ?: __('Invalid or expired API key', 'encountrix')),
			$code === 403 => sprintf(__('Access forbidden. Please verify API key permissions. Details: %s', 'encountrix'), $error_detail ?: __('Insufficient permissions', 'encountrix')),
			$code === 404 => sprintf(__('Resource not found. Please check raid name and parameters. Details: %s', 'encountrix'), $error_detail ?: __('The requested raid or guild was not found', 'encountrix')),
			$code === 429 => __('API rate limit exceeded. Please try again in a few minutes.', 'encountrix'),
			$code >= 500 && $code <= 503 => sprintf(__('API server error. The service may be temporarily unavailable. Details: %s', 'encountrix'), $error_detail ?: __('Please try again later', 'encountrix')),
			default => sprintf(__('API Error (HTTP %d): %s', 'encountrix'), $code, $error_detail ?: __('Unknown error occurred', 'encountrix')),
		};
	}

	public function sanitize_realm(string $realm): string {
		if (empty($realm)) { return ''; }
		if (strlen($realm) > 50) { return ''; }
	...
	public function sanitize_guilds(string|array $guilds): string {
		...
		if (!is_array($guilds)) { $guilds = preg_split('/[,\s]+/', (string) $guilds, -1, PREG_SPLIT_NO_EMPTY); }
		$guilds = array_map('sanitize_text_field', $guilds);
		$guilds = array_map('trim', $guilds);
		$guilds = array_filter($guilds, static function (string $g): bool { return (bool) preg_match('/^\d+$/', $g); });
		if (empty($guilds)) { return ''; }
	...
	public function get_realms_list(string $region = 'eu'): array {
		try {
			...
			$realms_url = add_query_arg(['namespace' => 'dynamic-' . $region, 'locale' => 'en_US'], "{$api_base}/data/wow/realm/index");
			...
			foreach ($data['realms'] as $realm) {
				if (isset($realm['slug'], $realm['name'])) {
					$realms[] = ['slug' => $realm['slug'], 'name' => $realm['name'], 'id' => $realm['id'] ?? 0];
				}
			}

			usort($realms, static function (array $a, array $b): int { return strcasecmp($a['name'], $b['name']); });
	...
	public function get_raid_icon_id(string $raid_slug, int $expansion_id): int|false {
		...
		if ($attachment_id && get_post($attachment_id)) { return (int) $attachment_id; }
	...
	public function get_raid_achievement_id(string $raid_slug, int $expansion_id): int|false {
		try {
			...
			$cached_id = get_transient($cache_key);
			if ($cached_id !== false) { return (int) $cached_id; }
			...
			$api_base = $this->regional_endpoints[$region]['api'];
			$achievements_url = add_query_arg(['namespace' => 'static-' . $region, 'locale' => 'en_US'], "{$api_base}/data/wow/achievement-category/{$category_id}");
			...
			$data = json_decode(wp_remote_retrieve_body($response), true);
			if (!isset($data['achievements'])) { return false; }

			$static_data = $this->fetch_static_raid_data_by_expansion($expansion_id, 1440);
			$raid_name = null;
			if (!is_wp_error($static_data) && isset($static_data['raids'])) {
				foreach ($static_data['raids'] as $raid) {
					if ($raid['slug'] === $raid_slug) { $raid_name = $raid['name']; break; }
				}
			}
			if (!$raid_name) { return false; }
	...
	private function download_raid_icon(string $raid_slug, int $achievement_id, int $expansion_id): int|false {
		try {
			...
			$api_base = $this->regional_endpoints[$region]['api'];
			$media_url = add_query_arg(['namespace' => 'static-' . $region, 'locale' => 'en_US'], "{$api_base}/data/wow/media/achievement/{$achievement_id}");
			...
			$file_array = ['name' => sanitize_file_name("raid_{$raid_slug}.jpg"), 'tmp_name' => $tmp];
	...
	public function get_journal_instance_id(string $raid_slug, int $expansion_id): int|false {
		try {
			...
			$api_base = $this->regional_endpoints[$region]['api'];
			$instances_url = add_query_arg(['namespace' => 'static-' . $region, 'locale' => 'en_US'], "{$api_base}/data/wow/journal-instance/index");
			...
			$data = json_decode(wp_remote_retrieve_body($response), true);
			if (!isset($data['instances'])) { return false; }
			...
			if (isset($this->tier_journal_map[$raid_slug])) {
				...
				foreach ($search_names as $search_name) {
					foreach ($data['instances'] as $instance) {
						if (!isset($instance['name'], $instance['id'])) { continue; }
						if (strcasecmp($instance['name'], $search_name) === 0 || stripos($instance['name'], $search_name) !== false) {
			...
			foreach ($data['instances'] as $instance) {
				if (isset($instance['name']) && (strcasecmp($instance['name'], $raid_name) === 0 || stripos($instance['name'], $raid_name) !== false)) {
					set_transient($cache_key, $instance['id'], 30 * DAY_IN_SECONDS);
					return (int) $instance['id'];
				}
			}
	...
	private function download_journal_instance_image(string $raid_slug, int $journal_id, int $expansion_id): int|false {
		...
		$api_base = $this->regional_endpoints[$region]['api'];
		$media_url = add_query_arg(['namespace' => 'static-' . $region, 'locale' => 'en_US'], "{$api_base}/data/wow/media/journal-instance/{$journal_id}");
		...
		if (isset($data['assets'])) {
			foreach ($data['assets'] as $asset) {
				if ($asset['key'] === 'tile') { $image_url = $asset['value']; break; }
			}
		}
		...
		$file_array = ['name' => sanitize_file_name("raid_bg_{$raid_slug}.jpg"), 'tmp_name' => $tmp];
	...
	public function get_expansions(): array { return $this->expansions; }
	public function get_regional_endpoints(): array { return $this->regional_endpoints; }
