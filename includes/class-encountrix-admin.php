...
class EncountrixAdmin {
	...
	public function register_settings(): void {
		// API Settings
		register_setting('encountrix_settings', 'encountrix_api_key', [
			'type' => 'string',
		...
		// Display Settings
		register_setting('encountrix_settings', 'encountrix_guild_ids', [
			'type' => 'string',
			'sanitize_callback' => [$this, 'sanitize_guilds'],
			'default' => ''
		]);

		register_setting('encountrix_settings', 'encountrix_expansion', [
			'type' => 'integer',
			'sanitize_callback' => [$this, 'sanitize_expansion'],
			'default' => 10
		]);

		register_setting('encountrix_settings', 'encountrix_raid', [
			'type' => 'string',
			'sanitize_callback' => 'sanitize_text_field',
			'default' => ''
		]);

		register_setting('encountrix_settings', 'encountrix_difficulty', [
			'type' => 'string',
			'sanitize_callback' => [$this, 'sanitize_difficulty'],
			'default' => 'highest'
		]);

		register_setting('encountrix_settings', 'encountrix_region', [
			'type' => 'string',
			'sanitize_callback' => [$this, 'sanitize_region'],
			'default' => 'eu'
		]);

		register_setting('encountrix_settings', 'encountrix_realm', [
			'type' => 'string',
			'sanitize_callback' => [$this, 'sanitize_realm'],
			'default' => ''
		]);

		register_setting('encountrix_settings', 'encountrix_cache_time', [
			'type' => 'integer',
			'sanitize_callback' => [$this, 'sanitize_cache_time'],
			'default' => 60,
		]);

		register_setting('encountrix_settings', 'encountrix_show_icons', [
			'type' => 'string',
			'sanitize_callback' => [$this, 'sanitize_boolean_string'],
			'default' => 'true'
		]);

		register_setting('encountrix_settings', 'encountrix_show_killed', [
			'type' => 'string',
			'sanitize_callback' => [$this, 'sanitize_boolean_string'],
			'default' => 'false'
		]);

		register_setting('encountrix_settings', 'encountrix_use_blizzard_icons', [
			'type' => 'string',
			'sanitize_callback' => [$this, 'sanitize_boolean_string'],
			'default' => 'true'
		]);

		register_setting('encountrix_settings', 'encountrix_show_raid_name', [
			'type' => 'string',
			'sanitize_callback' => [$this, 'sanitize_boolean_string'],
			'default' => 'false'
		]);

		register_setting('encountrix_settings', 'encountrix_show_raid_icon', [
			'type' => 'string',
			'sanitize_callback' => [$this, 'sanitize_boolean_string'],
			'default' => 'false'
		]);

		register_setting('encountrix_settings', 'encountrix_limit', [
			'type' => 'integer',
		...
		register_setting('encountrix_settings', 'encountrix_blizzard_client_id', [
			'type' => 'string',
			'sanitize_callback' => 'sanitize_text_field',
			'default' => ''
		]);

		register_setting('encountrix_settings', 'encountrix_blizzard_client_secret', [
			'type' => 'string',
			'sanitize_callback' => 'sanitize_text_field',
			'default' => ''
		]);

		register_setting('encountrix_settings', 'encountrix_blizzard_region', [
			'type' => 'string',
			'sanitize_callback' => [$this, 'sanitize_region'],
			'default' => 'eu'
		]);
		...
		// Cached raids
		register_setting('encountrix_settings', 'encountrix_cached_raids', [
			'type' => 'array',
			'sanitize_callback' => [$this, 'sanitize_cached_raids'],
			'default' => []
		]);

		register_setting('encountrix_settings', 'encountrix_debug_mode', [
			'type' => 'string',
			'sanitize_callback' => [$this, 'sanitize_boolean_string'],
			'default' => 'false'
		]);

		register_setting('encountrix_settings', 'encountrix_use_cron', [
			'type' => 'string',
			'sanitize_callback' => [$this, 'sanitize_boolean_string'],
			'default' => 'false'
		]);
	...
	public function ajax_import_icons(): void {
		...
		// Add summary
		$log[] = sprintf(
			__('Summary: %d imported, %d skipped, %d errors', 'encountrix'),
			$success_count,
			$skip_count,
			$error_count
		);
