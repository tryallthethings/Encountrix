...
class EncountrixAdmin {
	...
	public function register_settings(): void {
		register_setting('encountrix_settings', 'encountrix_api_key', ['type' => 'string', 'sanitize_callback' => 'sanitize_text_field', 'default' => '']);
		register_setting('encountrix_settings', 'encountrix_guild_ids', ['type' => 'string', 'sanitize_callback' => [$this, 'sanitize_guilds'], 'default' => '']);
		register_setting('encountrix_settings', 'encountrix_expansion', ['type' => 'integer', 'sanitize_callback' => [$this, 'sanitize_expansion'], 'default' => 10]);
		register_setting('encountrix_settings', 'encountrix_raid', ['type' => 'string', 'sanitize_callback' => 'sanitize_text_field', 'default' => '']);
		register_setting('encountrix_settings', 'encountrix_difficulty', ['type' => 'string', 'sanitize_callback' => [$this, 'sanitize_difficulty'], 'default' => 'highest']);
		register_setting('encountrix_settings', 'encountrix_region', ['type' => 'string', 'sanitize_callback' => [$this, 'sanitize_region'], 'default' => 'eu']);
		register_setting('encountrix_settings', 'encountrix_realm', ['type' => 'string', 'sanitize_callback' => [$this, 'sanitize_realm'], 'default' => '']);
		register_setting('encountrix_settings', 'encountrix_cache_time', ['type' => 'integer', 'sanitize_callback' => [$this, 'sanitize_cache_time'], 'default' => 60]);
		register_setting('encountrix_settings', 'encountrix_show_icons', ['type' => 'string', 'sanitize_callback' => [$this, 'sanitize_boolean_string'], 'default' => 'true']);
		register_setting('encountrix_settings', 'encountrix_show_killed', ['type' => 'string', 'sanitize_callback' => [$this, 'sanitize_boolean_string'], 'default' => 'false']);
		register_setting('encountrix_settings', 'encountrix_use_blizzard_icons', ['type' => 'string', 'sanitize_callback' => [$this, 'sanitize_boolean_string'], 'default' => 'true']);
		register_setting('encountrix_settings', 'encountrix_show_raid_name', ['type' => 'string', 'sanitize_callback' => [$this, 'sanitize_boolean_string'], 'default' => 'false']);
		register_setting('encountrix_settings', 'encountrix_show_raid_icon', ['type' => 'string', 'sanitize_callback' => [$this, 'sanitize_boolean_string'], 'default' => 'false']);
		register_setting('encountrix_settings', 'encountrix_limit', ['type' => 'integer', 'sanitize_callback' => [$this, 'sanitize_limit'], 'default' => 50]);
		register_setting('encountrix_settings', 'encountrix_blizzard_client_id', ['type' => 'string', 'sanitize_callback' => 'sanitize_text_field', 'default' => '']);
		register_setting('encountrix_settings', 'encountrix_blizzard_client_secret', ['type' => 'string', 'sanitize_callback' => 'sanitize_text_field', 'default' => '']);
		register_setting('encountrix_settings', 'encountrix_blizzard_region', ['type' => 'string', 'sanitize_callback' => [$this, 'sanitize_region'], 'default' => 'eu']);
		register_setting('encountrix_settings', 'encountrix_cached_raids', ['type' => 'array', 'sanitize_callback' => [$this, 'sanitize_cached_raids'], 'default' => []]);
		register_setting('encountrix_settings', 'encountrix_debug_mode', ['type' => 'string', 'sanitize_callback' => [$this, 'sanitize_boolean_string'], 'default' => 'false']);
		register_setting('encountrix_settings', 'encountrix_use_cron', ['type' => 'string', 'sanitize_callback' => [$this, 'sanitize_boolean_string'], 'default' => 'false']);
	}
	...
	public function sanitize_cache_time(mixed $value): int { return min(1440, max(0, absint($value))); }
	public function sanitize_limit(mixed $value): int { return min(100, max(1, absint($value))); }

	public function sanitize_boolean_string(mixed $value): string {
		return in_array((string) $value, ['true', 'false'], true) ? (string) $value : 'false';
	}

	public function sanitize_cached_raids(mixed $value): array { return is_array($value) ? $value : []; }
	public function sanitize_realm(mixed $value): string { return $this->api->sanitize_realm((string) $value); }
	public function sanitize_guilds(mixed $value): string { return $this->api->sanitize_guilds($value ?? ''); }

	public function enqueue_admin_assets(string $hook): void {
		if ($hook !== 'settings_page_encountrix') { return; }

		wp_enqueue_style('encountrix-admin', ENCOUNTRIX_PLUGIN_URL . 'assets/css/admin.css', [], $this->version);
		wp_enqueue_script('encountrix-admin', ENCOUNTRIX_PLUGIN_URL . 'assets/js/admin.js', ['jquery'], $this->version, true);

		wp_localize_script('encountrix-admin', 'encountrix_admin', [
	...
	public function ajax_clear_cache(): void {
		if (!check_ajax_referer('encountrix_admin_nonce', 'nonce', false)) { wp_send_json_error(__('Security check failed', 'encountrix')); return; }
		if (!current_user_can('manage_options')) { wp_send_json_error(__('Permission denied', 'encountrix')); return; }
		$this->delete_transients_by_prefix(['_transient_encountrix_', '_transient_timeout_encountrix_', '_transient_encountrix_blizzard_', '_transient_timeout_encountrix_blizzard_']);
	...
	public function ajax_import_icons(): void {
		...
		if (empty($raid_slug)) { wp_send_json_error(__('No raid selected in settings', 'encountrix')); return; }
		...
		if (is_wp_error($static_data) || !isset($static_data['raids'])) { wp_send_json_error(__('Could not fetch raid data', 'encountrix')); return; }

		$raid_info = null;
		foreach ($static_data['raids'] as $raid) { if ($raid['slug'] === $raid_slug) { $raid_info = $raid; break; } }
		if (!$raid_info || !isset($raid_info['encounters'])) {
			wp_send_json_error(sprintf(__('Could not find raid: %s', 'encountrix'), $raid_slug)); return;
		}

		$log = []; $success_count = 0; $skip_count = 0; $error_count = 0;

		foreach ($raid_info['encounters'] as $boss) {
			$result = $this->api->import_single_boss_icon($boss['slug'], $boss['name'], $raid_slug, $expansion_id);
			if (is_int($result)) {
				$success_count++;
				$log[] = sprintf(__('SUCCESS: Imported icon for %s (ID: %d)', 'encountrix'), $boss['name'], $result);
			} elseif ($result === true) {
				$skip_count++;
			...
			if ($success_count > 0 && $success_count % 3 === 0) { usleep(300000); }
		}

		$log[] = sprintf(__('Summary: %d imported, %d skipped, %d errors', 'encountrix'), $success_count, $skip_count, $error_count);
	...
	public function ajax_delete_icons(): void {
		...
		wp_send_json_success(sprintf(__('Successfully deleted %d icon(s) from the media library', 'encountrix'), $deleted));
	}
	...
	public function ajax_get_raids(): void {
		...
		$valid_expansions = array_keys($this->api->get_expansions());
		if (!in_array($expansion_id, $valid_expansions, true)) { wp_send_json_error(__('Invalid expansion ID', 'encountrix')); return; }

		$cached_raids = get_option('encountrix_cached_raids', []);
		if (isset($cached_raids[$expansion_id]) && !empty($cached_raids[$expansion_id])) {
			...
			wp_send_json_success($cached_raids[$expansion_id]); return;
		}

		$this->api->debug_log("Fetching raids for expansion $expansion_id from API");
		$static_data = $this->api->fetch_static_raid_data_by_expansion($expansion_id, 1440);
		if (is_wp_error($static_data) || !isset($static_data['raids'])) { wp_send_json_error(__('Could not fetch raids', 'encountrix')); return; }

		$raids = [];
		foreach ($static_data['raids'] as $raid) { $raids[] = ['slug' => $raid['slug'], 'name' => $raid['name']]; }
	...
	public function ajax_refresh_raids(): void {
		...
		$this->delete_transients_by_prefix(['_transient_encountrix_static_', '_transient_timeout_encountrix_static_']);
		$this->ajax_get_raids();
	}
	...
	public function ajax_get_realms(): void {
		...
		if (!in_array($region, ['us', 'eu', 'kr', 'tw'], true)) { wp_send_json_error(__('Invalid region specified', 'encountrix')); return; }
		$this->api->debug_log("Fetching realms for region: $region");
		$realms = $this->api->get_realms_list($region);
		if (empty($realms)) { wp_send_json_error(__('Could not fetch realms. Check Blizzard API settings.', 'encountrix')); return; }
